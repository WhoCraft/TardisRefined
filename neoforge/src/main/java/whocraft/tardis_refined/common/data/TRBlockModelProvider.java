...
public class TRBlockModelProvider extends BlockStateProvider {
    ...
    public JsonObject emptyBlockState(Block block) {
        VariantBlockStateBuilder builder = getVariantBuilder(block).forAllStates(blockState -> ConfiguredModel.builder().modelFile(models().getExistingFile(ResourceLocation.tryBuild(TardisRefined.MODID, "block/terraformer"))).build());
    ...
    public JsonObject terraformer(Block block) {
        VariantBlockStateBuilder builder = getVariantBuilder(block).forAllStates(blockState -> blockState.getValue(TerraformerBlock.ACTIVE) ?
                ConfiguredModel.builder().modelFile(models().getExistingFile(ResourceLocation.tryBuild(TardisRefined.MODID, "block/terraformer_on"))).build() :
                ConfiguredModel.builder().modelFile(models().getExistingFile(ResourceLocation.tryBuild(TardisRefined.MODID, "block/terraformer"))).build());
    ...
    public JsonObject lantern(Block block) {
        VariantBlockStateBuilder builder = getVariantBuilder(block).forAllStates(blockState -> blockState.getValue(LanternBlock.HANGING) ?
                ConfiguredModel.builder().modelFile(models().withExistingParent("block/zeiton_lantern_hanging", ResourceLocation.tryBuild("block/template_hanging_lantern")).texture("lantern", ResourceLocation.tryBuild(TardisRefined.MODID, "block/zeiton_lantern"))).build() :
                ConfiguredModel.builder().modelFile(models().withExistingParent("block/zeiton_lantern", ResourceLocation.tryBuild("block/template_lantern")).texture("lantern", ResourceLocation.tryBuild(TardisRefined.MODID, "block/zeiton_lantern"))).build());
    ...
    public ResourceLocation getBlockTextureResourceLocation(Block block) {
        ...
        return ResourceLocation.tryBuild(TardisRefined.MODID, "block/" + blockTex.getPath());
    }
    ...
    public JsonObject antiGravityBlock(Block block) {
        ...
        ResourceLocation modelLocation0 = ResourceLocation.tryBuild(TardisRefined.MODID, "block/gravity_well");

        for (int space = 0; space <= 5; space++) {
            ResourceLocation modelLocation = space == 0 ? modelLocation0 : ResourceLocation.tryBuild(TardisRefined.MODID, "block/gravity_well_" + space);
    ...
    protected void registerStatesAndModels() {
        ...
        ResourceLocation growthStoneTexture = ResourceLocation.tryBuild("tardis_refined:block/growth_stone");
...
*/
        ...
        threeDeeRotating(TRBlockRegistry.LANDING_PAD.get(), ResourceLocation.tryBuild(TardisRefined.MODID, "block/landing_pad"));
        threeDeeRotating(TRBlockRegistry.FLIGHT_DETECTOR.get(), ResourceLocation.tryBuild(TardisRefined.MODID, "block/flight_detector"));
       ...
       // simpleBlock(TRBlockRegistry.ASTRAL_MAP.get());
        ...
        customLocation(TRBlockRegistry.ASTRAL_MANIPULATOR_BLOCK.get(), ResourceLocation.tryBuild(TardisRefined.MODID, "block/astral_manipulator"));
        customLocation(TRBlockRegistry.CORRIDOR_TELEPORTER.get(), ResourceLocation.tryBuild(TardisRefined.MODID, "block/corridor_teleporter"));
        ...
        threeDeeRotating(TRBlockRegistry.CONSOLE_CONFIGURATION_BLOCK.get(), ResourceLocation.tryBuild(TardisRefined.MODID, "block/console_configuration"));

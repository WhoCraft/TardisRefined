...
public class TRFabricBiomeModifiers {
    ...
    private static void addToBiome(String featureName, String biomeKeyName, GenerationStep.Decoration step) {
        BiomeModifications.create(ResourceLocation.tryBuild(TardisRefined.MODID, featureName))
                .add(ModificationPhase.ADDITIONS,
                        (context) -> context.hasTag(TagKey.create(Registries.BIOME, ResourceLocation.tryBuild(TardisRefined.MODID, biomeKeyName))),
                        (context) -> context.getGenerationSettings().addFeature(step, ResourceKey.create(Registries.PLACED_FEATURE, ResourceLocation.tryBuild(TardisRefined.MODID, featureName))));
    }
}
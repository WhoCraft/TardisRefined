...
public class HumSelectionScreen extends MonitorOS {
    ...
    public HumSelectionScreen() {
        super(Component.translatable(ModMessages.UI_MONITOR_SELECT_HUM), ResourceLocation.fromNamespaceAndPath(TardisRefined.MODID, "textures/gui/monitor/backdrop.png"));
    }

    public static void selectHum(HumEntry theme) {
        assert Minecraft.getInstance().player != null;
        NetworkManager.get().sendToServer(new C2SChangeHum(Minecraft.getInstance().player.level().dimension(), theme));
    ...
    public ObjectSelectionList<SelectionListEntry> createSelectionList() {
        ...
        for (HumEntry humEntry : knownHums) {
            ...
            try {
                name = Component.Serializer.fromJson(humEntry.getNameComponent(), minecraft.level.registryAccess());
